...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for(int row = 0; row < len; row++){
            for(int col = 0; col < len; col++){
                if(row < len - 1 && b.tile(row, col).value() == b.tile(row + 1, col).value()){
                    return true;
                }
                if(col < len - 1 && b.tile(row, col).value() == b.tile(row, col + 1).value()){
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        for(int row = y; row < b.size() - 1; row++){
            if(b.tile(x, row + 1) == null){
                targetY++;
            }else if(!b.tile(x, row + 1).wasMerged() && b.tile(x, row + 1).value() == myValue){
                    targetY++;
                    score += myValue * 2;
                    break;
            }else{
        ...
        if (targetY != y) {  // Only move if targetY has changed
    ...
    public void tiltColumn(int x) {
        ...
        for (int row = this.getBoard().size() - 2; row >= 0; row--) {
    ...
    public void tilt(Side side) {
        ...
        this.getBoard().setViewingPerspective(side);
        for(int col = 0; col < this.getBoard().size(); col++){
            tiltColumn(col);
        }
        this.getBoard().setViewingPerspective(Side.NORTH);
    }
